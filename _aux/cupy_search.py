...
def calc_partition_lens(partition_by, nchain):
    """
    Takes a sorted array partition_by and calculates the length of the blocks of repeating elements.
    """
    if partition_by.ndim != 1:
    ...
    if len(diff_inds) != nchain:
        # Then find the index where each of the values in [0, nchain] would belong:
        insert_at   = cupy.searchsorted(partition_by[diff_inds], cupy.arange(nchain))
    ...
    lens        = cupy.empty(len(diff_inds), dtype=cupy.uint32 if len(partition_by) < 2**32 else cupy.uint64)
...
def find_changes_local_single(arr, mindiff=0):
    """
    Takes a (sorted) 1D array and returns the segmentation indices, i.e. the indices where the value changes.
    """
...
#def searchsorted_multidim_list(phonebook, findme, mindiff=64, allow_escapes=False, linear_only=False, shutup=True):
#    return searchsorted_binary_only(phonebook, findme, allow_escapes=False)

####################################################################################################################################################################################

def searchsorted_multidim_list_vanilla(phonebook, findme, mindiff=32, allow_escapes=False, linear_only=False, shutup=True):
    return searchsorted_multidim_list_old(phonebook, findme, mindiff, allow_escapes, linear_only, shutup).directcalc()

def searchsorted_multidim_list_8bits(phonebook, findme, mindiff=32, allow_escapes=False, linear_only=False, shutup=True):
    return searchsorted_multidim_list_old(phonebook, findme, mindiff, allow_escapes, linear_only, shutup).parseto8bits()


def _check_if_trivial(phonebook, findme, allow_escapes):
    if findme.shape[0]      == 0:
        return cupy.array([], dtype=cupy.uint32)
    elif phonebook.shape[0]   == 1:
...
class searchsorted_multidim_list_old:
    def __init__(self, phonebook, findme, mindiff, allow_escapes, linear_only, shutup):
        ...
        if phonebook.shape[0] >= 2**32-1 or phonebook.shape[1] >= 2**16:
            raise NotImplementedError("This function will not work with arrays whose dimensions exceed (2**32 - 2, 2**16 - 1).")
        if phonebook.dtype != findme.dtype:
            raise TypeError("Mismatched dtypes: phonebook has dtype %s and findme has dtype %s." % (phonebook.dtype, findme.dtype))        
        if phonebook.dtype not in (cupy.uint8, cupy.uint32):
            raise NotImplementedError("dtype %s for phonebook and findme is not supported." % (phonebook.dtype, findme.dtype))
    ...
    def parseto8bits(self):
        ...
        for colnum in it_arr:
            if not self.shutup:
                print("Column %i..." % colnum, end=' ')
            thisbook, thisfind  = self.phonebook.view("u1")[:,colnum], self.findme.view("u1")[:,colnum]
...
##########################################################################################
    ...
    def _preprocess(self):
        if self.phonebook.shape[0] <= self.mindiff:
            self.linear_only = True      # this is the global override option for use_binary; if this is True, then the diffs will not be computed and use_binary will always be False
        if self.linear_only:
    ...
    def _single_loop(self, result_inds, thisbook, thisfind):
        shutup  = self.shutup
        if not self.linear_only:                 # Check whether we should perform a binary search in the current step:
            t0 = time.time()
            thisdiff            = find_changes_local_single(thisbook, mindiff=self.mindiff)
            if len(thisdiff) == 1:
                if not shutup:
                    print("Constant column condition triggered.")#, calc took %f ms." % ((time.time() - t0) * 1000))
                ...
                if cupy.any(escapes):
                    if self.allow_escapes:
                        ...
                        raise RuntimeError("Column contains only one value which does not match the requested values.")
#                continue        # skip the rest of the loop if this condition was satisfied
                return result_inds
            diffarr = cupy.diff(thisdiff)
            if diffarr.max() >= self.mindiff:    # use a binary search in thisdiff to find the next partner to check
                if not shutup:
                    print("Binary search...", end=' ')
                self.use_binary = True
            else:                           # else do a linear search in thisbook and check every single partner (if not use_binary and not constant column)
                if not shutup:
                    print("Linear search...", end=' ')
                self.use_binary = False
            del diffarr
            if not shutup:
                print("Thisdiff and method determination took %f ms." % ((time.time() - t0)*1000), end=' ')
        t0 = time.time()
        previous_inds = result_inds.copy()
        if self.use_binary:
            self.skib(thisfind, thisbook, previous_inds, thisdiff, self.allow_escapes, cupy.array([len(thisdiff)], dtype=cupy.uint32), result_inds)
        elif not self.linear_only:
            self.skil_imp(thisfind, thisbook, previous_inds, self.allow_escapes, cupy.array([len(thisbook)], dtype=cupy.uint32), cupy.array([thisdiff[-1]], dtype=cupy.uint32), result_inds)
        else:
            self.skil(thisfind, thisbook, previous_inds, self.allow_escapes, cupy.array([len(thisbook)], dtype=cupy.uint32), result_inds)
        if not shutup:
            t1 = time.time()
            print("Search & sync took: %f ms." % ((t1 - t0) * 1000), end=' ')
            if t1 - t0 > 1:
        ...
        if not shutup:
            print("Copy: %f ms." % ((time.time() - t1) * 1000))
...
_searchkernel_individual_linear8 = cupy.ElementwiseKernel(
    'uint8 findme, raw uint8 phonebook, raw uint32 startfrom, bool allow_escapes, raw uint32 len_phonebook',
    'uint32 y',
    skil_code,
    name="searchkernel_individual_linear8")

_searchkernel_individual_linear32 = cupy.ElementwiseKernel(
    'uint32 findme, raw uint32 phonebook, raw uint32 startfrom, bool allow_escapes, raw uint32 len_phonebook',
    'uint32 y',
    skil_code,
    name="searchkernel_individual_linear32")

_searchkernel_individual_linear_improved8 = cupy.ElementwiseKernel(
    'uint8 findme, raw uint8 phonebook, raw uint32 startfrom, bool allow_escapes, raw uint32 len_phonebook, raw uint32 last_diff_ind',
    'uint32 y',
    skil_improved_code,
    name="searchkernel_individual_linear_improved8")

_searchkernel_individual_linear_improved32 = cupy.ElementwiseKernel(
    'uint32 findme, raw uint32 phonebook, raw uint32 startfrom, bool allow_escapes, raw uint32 len_phonebook, raw uint32 last_diff_ind',
    'uint32 y',
    skil_improved_code,
    name="searchkernel_individual_linear_improved32")

_searchkernel_individual_binary8 = cupy.ElementwiseKernel(
    'uint8 findme, raw uint8 phonebook, raw uint32 startfrom, raw uint32 difflist, bool allow_escapes, raw uint32 n_diffs',
    'uint32 y',
    skib_code,
    name="searchkernel_individual_binary8")

_searchkernel_individual_binary32 = cupy.ElementwiseKernel(
    'uint32 findme, raw uint32 phonebook, raw uint32 startfrom, raw uint32 difflist, bool allow_escapes, raw uint32 n_diffs',
    'uint32 y',
    skib_code,
...
def _verify_binary_input(phonebook, findme):
    if not (phonebook.flags["C_CONTIGUOUS"] and findme.flags["C_CONTIGUOUS"]):
        raise ValueError("The modified search routine only works with C-contiguous basis_states arrays (consider calling cupy.ascontiguousarray() on the input arrays first).")
    if phonebook.dtype != findme.dtype:
        raise TypeError("Mismatched dtypes: phonebook has dtype %s and findme has dtype %s." % (phonebook.dtype, findme.dtype))        
...
def _binary_prepare_args(findme, phonebook):
    ...
    funcargs            = [(blocks,), (threads_per_block,), (findme, phonebook, cupy.uint32(phonebook.shape[0]), *[cupy.uint32(i) for i in findme.shape], result)]
    return funcargs
...
class _binary_constructor_codes:
    def __init__(self):
        #############################################
        # base code with left_check
        #############################################
        self.binary_only_raw_check_left = r'''
            extern "C" __global__
            void METHODNAME(const unsigned int* findme, const unsigned int* phonebook,
                const unsigned int phonebooknum, const unsigned int findmenum, const unsigned int rowlen,
                unsigned int* result)
            {
                    ...
                    while (left < right) {
                        ...
                        for (col = 0; col < rowlen; ++col) {
                            if (phonebook[m*rowlen + col] > findme[arrind + col]) {
                                col = rowlen + 1;   /* use as magic number to indicate that phonebook[m] > findme */
                                break;
                            }
        ...
        #############################################
        self.binary_only_no_escape_EOA = r'''
                            printf("Error! Reached end of array while searching for %u.\n", findme[arrind]);
                            assert(0);
            '''

        self.binary_only_no_escape_NOMATCH = r'''
                            if (left > 0) {
                                printf("Error! Went from %u to %u. Searching for: %u.\n", phonebook[(left-1)*rowlen + col], phonebook[left*rowlen + col], findme[arrind]);
                            } else {
                                printf("Error! Searching for %u, but smallest entry is %u.\n", findme[arrind], phonebook[left*rowlen + col]);
                            }
                            assert(0);
            '''

        self.binary_only_no_escape_OVERRUN = r'''
                            printf("Error! Went from %u to %u. Searching for: %u.\n", phonebook[(left-1)*rowlen + col], phonebook[left*rowlen + col], findme[arrind]);
                            assert(0);
            '''

        self.binary_only_no_escape_LEFTCHE = r'''
        ...
        #############################################
        self.binary_only_escape_EOA = r'''
                            result[tid] = 4294967295;
                            return;
            '''
        self.binary_only_escape_NOMATCH = self.binary_only_escape_EOA
        self.binary_only_escape_OVERRUN = self.binary_only_escape_EOA
        self.binary_only_escape_LEFTCHE = self.binary_only_escape_EOA



##########################################################################################

def _process_code(constr_obj, methname, escapes, left_check):
    TRIGGERNAMES    = ["END_OF_ARRAY_TRIGGER", "NO_MATCH_TRIGGER", "OVERRUN_TRIGGER"]
    codenames       = ["EOA", "NOMATCH", "OVERRUN"]
    if left_check:
        basecode        = constr_obj.binary_only_raw_check_left
        TRIGGERNAMES    += ["LEFT_CHECK_TRIGGER",]
        codenames       += ["LEFTCHE",]
    else:
        basecode    = constr_obj.binary_only_raw

    if escapes:
        codes       = [getattr(constr_obj, "binary_only_escape_%s" % s) for s in codenames]
    else:
        codes       = [getattr(constr_obj, "binary_only_no_escape_%s" % s) for s in codenames]
...
constr_obj = _binary_constructor_codes()
...
_searchkernel_full_binary_noesc = cupy.RawKernel(
    constr_obj.full_binary_raw.replace("METHODNAME", noescfullbinname).replace("ASSERT_VALUE_SNIPPET", constr_obj.full_binary_assert),
    noescfullbinname)

fullbinname         = "full_binary"
_searchkernel_full_binary = cupy.RawKernel(
    constr_obj.full_binary_raw.replace("METHODNAME", fullbinname).replace("ASSERT_VALUE_SNIPPET", ''),
    fullbinname)
