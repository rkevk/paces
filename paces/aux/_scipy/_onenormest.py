...
from cupyx.scipy.sparse.linalg import aslinearoperator  # pylint: disable=import-error
...
def onenormest(A, t=2, itmax=5, compute_v=False, compute_w=False):
    """
    Compute a lower bound of the 1-norm of a sparse array. See scipy.sparse.linalg.onenormest.
    ...
    n = A.shape[1]
    if t >= n:
        A_explicit = cupy.asarray(aslinearoperator(A).matmat(cupy.identity(n)))
        ...
        argmax_j = cupy.argmax(col_abs_sums)
...
def _blocked_elementwise(func):
    ...
    def wrapper(x):
        if x.shape[0] < block_size:
            ...
            y = cupy.zeros((x.shape[0],) + y0.shape[1:], dtype=y0.dtype)
            y[:block_size] = y0
            del y0
...
def _max_abs_axis1(X):
    return cupy.max(cupy.abs(X), axis=1)
...
def _sum_abs_axis0(X):
    ...
    for j in range(0, X.shape[0], block_size):
        y = cupy.sum(cupy.abs(X[j:j+block_size]), axis=0)
        if r is None:
            r = y
        else:
            r += y
...
def resample_column(i, X):
    X[:, i] = cupy.random.randint(0, 2, size=X.shape[0])*2 - 1
...
def _onenormest_core(A, AT, t, itmax):
    ...
    ind_hist = cupy.zeros(0, dtype=cupy.intp)
    est_old = 0
    S = cupy.zeros((n, t), dtype=float)
    k = 1
    ind = None
    while True:
        Y = cupy.asarray(A_linear_operator.matmat(X))
        nmults += 1
        mags = _sum_abs_axis0(Y)
        est = cupy.max(mags)
        best_j = cupy.argmax(mags)
        ...
        del S_old
        # (3)
        Z = cupy.asarray(AT_linear_operator.matmat(S))
        nmults += 1
        h = _max_abs_axis1(Z)
        del Z
        # (4)
        ...
        ind = cupy.argsort(h)[::-1][:t+len(ind_hist)].copy()
        del h
        if t > 1:
            # (5)
            # Break if the most promising t vectors have been visited already.
            if cupy.isin(ind[:t], ind_hist).all():
                break
            ...
            seen = cupy.isin(ind, ind_hist)
            ind = cupy.concatenate((ind[~seen], ind[seen]))
        for j in range(t):
            X[:, j] = elementary_vector(n, ind[j])

        new_ind = ind[:t][~cupy.isin(ind[:t], ind_hist)]
        ind_hist = cupy.concatenate((ind_hist, new_ind))
        k += 1
